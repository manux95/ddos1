...
if ($help || !$ip) {
 ...
 udp oficial da OnFall Team

 Para utilizar use:
 perl udp.pl IP PORTA 65500

 https://www.youtube.com/channel/UCcUxoMMqO4a3sg-kdH4mx9g
...
print "Atacando ip: $ip " . ($port ? $port : "By:") . " XxASxX_YBanY " . 
  ($size ? "$size-byte" : "UDP Oficial da: OnFall Team") . " www.youtube.com/user/XxASxXYBanY" . ($time ? " for $time seconds" : "") . "\n";
